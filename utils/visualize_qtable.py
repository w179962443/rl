...
def visualize_q_table(agent, save_path='results/plots/q_table.png'):
    ...
    for idx, (ax, action_name) in enumerate(zip(axes.flat, actions)):
        ...
        ax.set_title(f'Q-values for Action: {action_name}', fontsize=14, fontweight='bold')
...
def visualize_policy(agent, save_path='results/plots/policy.png'):
    ...
    for i in range(4):
        for j in range(4):
            ...
            # Add cell label
            ax.text(j, 3-i+0.3, cell_type, ha='center', va='center',
            ...
            if cell_type not in ['H', 'G']:
                ...
                ax.text(j, 3-i-0.1, symbol, ha='center', va='center',
    ...
    ax.set_title('Learned Policy for FrozenLake\n(Arrows show best action)', 
                fontsize=14, fontweight='bold')
...
def visualize_value_function(agent, save_path='results/plots/value_function.png'):
    ...
    for i in range(4):
        for j in range(4):
            text = ax.text(j, i, f'{state_values[i, j]:.3f}',
    ...
    ax.set_xlabel('Column', fontsize=12)
    ax.set_ylabel('Row', fontsize=12)
    ax.set_title('State Value Function (V(s) = max Q(s,a))', 
                fontsize=14, fontweight='bold')
