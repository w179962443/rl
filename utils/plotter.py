...
class Plotter:
    ...
    def plot_training_progress(self, rewards, losses=None, window=100, filename='training_progress.png'):
        """
        ...
        """
        fig, axes = plt.subplots(1, 2 if losses else 1, figsize=(15 if losses else 10, 5))
        ...
        if losses:
            ...
            if non_zero_losses:
                ...
                if len(loss_values) >= window:
                    loss_moving_avg = np.convolve(loss_values, np.ones(window)/window, mode='valid')
                    axes[1].plot(range(indices[window-1], indices[-1]+1), loss_moving_avg,
                               label=f'{window}-Step Moving Avg', linewidth=2)
    ...
    def plot_comparison(self, data_dict, ylabel='Reward', title='Comparison', 
                       filename='comparison.png', window=100):
        """
