# Super Mario Bros Reinforcement Learning Experiment

训练AI玩超级马里奥兄弟游戏！

## 🎮 游戏特性

- **环境**: Nintendo Entertainment System (NES) 超级马里奥
- **关卡**: World 1-1 到 8-4
- **算法**: CNN-DQN (卷积神经网络 DQN)
- **状态**: 4帧堆叠的84x84灰度图像
- **动作**: 7个简单动作（右、右+跳、跳等）

## 📁 目录结构

```
experiments/mario/
├── train.py          # 训练脚本
├── test.py           # 测试脚本
├── models/           # 保存的模型
│   ├── best_score_model.pth      # 最高分模型
│   ├── best_distance_model.pth   # 最远距离模型
│   └── checkpoint_ep{N}.pth      # 定期检查点
└── logs/             # 训练日志和图表
```

## 🚀 使用方法

### 训练

```bash
# 训练 World 1-1
python train.py --game mario --episodes 10000 --render-every 100

# 训练特定关卡
python train.py --game mario --episodes 10000 --world 1 --stage 2
```

**训练参数：**
- `--episodes`: 训练回合数（建议10000+）
- `--render-every`: 每N轮渲染一次
- `--world`: 世界编号 (1-8)
- `--stage`: 关卡编号 (1-4)

### 测试

```bash
# 测试最佳分数模型
python test.py --game mario --model experiments/mario/models/best_score_model.pth --episodes 5

# 测试最远距离模型
python test.py --game mario --model experiments/mario/models/best_distance_model.pth --episodes 5

# 测试特定关卡
python test.py --game mario --world 1 --stage 1 --model {path} --episodes 5
```

## 📊 训练输出

训练过程会生成：

1. **模型文件**
   - `best_score_model.pth` - 最高分数模型
   - `best_distance_model.pth` - 到达最远位置的模型
   - `checkpoint_episode_{N}.pth` - 定期检查点

2. **训练图表**
   - 分数曲线
   - 最远距离曲线
   - Epsilon探索率
   - 训练损失

3. **训练历史**
   - `training_history.pkl` - 完整训练数据

## 🎯 训练目标

### 短期目标（前1000轮）
- 学习基本移动（向右走）
- 避开简单障碍物
- 跳过小坑

### 中期目标（1000-5000轮）
- 跳过多个障碍
- 击败敌人
- 到达更远距离

### 长期目标（5000+轮）
- 完成关卡
- 获得高分
- 稳定通关

## ⚙️ 网络架构

**CNN-DQN网络：**
```
输入: 4 x 84 x 84 (4帧堆叠)
  ↓
Conv1: 32 filters, 8x8, stride 4 → ReLU
  ↓
Conv2: 64 filters, 4x4, stride 2 → ReLU
  ↓
Conv3: 64 filters, 3x3, stride 1 → ReLU
  ↓
Flatten
  ↓
FC1: 512 units → ReLU
  ↓
FC2: 7 units (动作)
```

## 💡 训练技巧

1. **耐心很重要** - CNN-DQN需要大量训练才能学会
2. **GPU加速** - 强烈建议使用GPU训练
3. **观察进度** - 监控最远距离而不只是分数
4. **调整探索** - Epsilon衰减很慢，确保充分探索
5. **从简单关卡开始** - World 1-1 是最好的起点

## 🔧 超参数

```python
{
    "gamma": 0.99,              # 折扣因子
    "learning_rate": 0.00025,   # 学习率
    "batch_size": 32,           # 批次大小
    "memory_size": 100000,      # 经验回放大小
    "target_update_freq": 10000, # 目标网络更新频率
    "epsilon_start": 1.0,       # 初始探索率
    "epsilon_end": 0.1,         # 最小探索率
    "epsilon_decay": 0.999995,  # 探索率衰减
}
```

## 📈 预期性能

| 训练轮数 | 预期表现 |
|---------|---------|
| 0-500 | 学习向右移动 |
| 500-2000 | 跳过简单障碍，到达X=500+ |
| 2000-5000 | 避开敌人，到达X=1000+ |
| 5000-10000 | 可能通过部分关卡 |
| 10000+ | 稳定到达远距离，偶尔通关 |

## ⚠️ 注意事项

1. **训练时间长** - 可能需要数小时甚至数天
2. **GPU推荐** - CPU训练会非常慢
3. **内存占用** - 100k经验回放需要较多内存
4. **渲染影响** - 频繁渲染会减慢训练速度

## 🎓 学习资源

- [gym-super-mario-bros 文档](https://github.com/Kautenja/gym-super-mario-bros)
- [DQN 论文](https://www.nature.com/articles/nature14236)
- [CNN-DQN 架构](https://arxiv.org/abs/1312.5602)

---

**准备好训练你的马里奥AI了吗？** 🍄🎮
