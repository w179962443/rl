...
def train_cartpole(args):
    ...
    for episode in range(args.episodes):
        ...
        logger.log_episode(episode, episode_reward, steps, epsilon, episode_loss / steps)
        
        # Print progress
        if episode % 10 == 0:
            avg_reward = np.mean(rewards_history[-10:])
            logger.print_episode(episode, avg_reward, steps, epsilon, episode_loss / steps)
    ...
    logger.save()
    plotter.plot_training_progress(rewards_history, losses_history, 
                                   filename='cartpole_training.png')
...
def train_frozenlake(args):
    ...
    logger.save()
    plotter.plot_training_progress(rewards_history, window=100,
                                   filename='frozenlake_training.png')
...
def train_snake(args):
    ...
    for episode in range(args.episodes):
        ...
        rewards_history.append(episode_reward)
        losses_history.append(episode_loss / steps if steps > 0 else 0)
        logger.log_episode(episode, episode_reward, steps, epsilon, episode_loss / steps if steps > 0 else 0)
        
        # Print progress
        if episode % 10 == 0:
            avg_reward = np.mean(rewards_history[-10:])
            logger.print_episode(episode, avg_reward, steps, epsilon, episode_loss / steps if steps > 0 else 0)
...
def main():
    ...
    parser.add_argument('--game', type=str, required=True,
                       choices=['cartpole', 'pong', 'frozenlake', 'snake'],
